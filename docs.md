...
| `visibleRange`           | `{`<br />`x: { min: number, max: number },`<br />`y: {`<br />`left: { min: number, max: number },`<br />`right: { min: number, max: number }`<br />`}`<br />`}` |         | When the dataset has fewer entries than `x.min`, the chart keeps the same zoom level as if `x.min` entries were present. |
