//
//  RNCandleStickChartComponentView.mm
//  Fabric ComponentView wrapper for `RNCandleStickChartView` (Swift).
//
...
@interface RNCandleStickChartComponentView : RCTViewComponentView
@end

@implementation RNCandleStickChartComponentView {
  RNCandleStickChartView *_swiftView;
}

+ (ComponentDescriptorProvider)componentDescriptorProvider
{
  return concreteComponentDescriptorProvider<RNCandleStickChartComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const RNCandleStickChartProps>();
    _props = defaultProps;

    _swiftView = [[RNCandleStickChartView alloc] initWithFrame:self.bounds];
...
{
  const auto &newProps = *std::static_pointer_cast<RNCandleStickChartProps const>(props);
  const auto &oldP = oldProps
                         ? *std::static_pointer_cast<RNCandleStickChartProps const>(oldProps)
...
Class<RCTComponentViewProtocol> RNCandleStickChartComponentViewCls(void)
{
  return RNCandleStickChartComponentView.class;
}
