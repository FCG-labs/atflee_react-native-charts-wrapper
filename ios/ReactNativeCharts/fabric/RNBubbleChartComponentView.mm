//
//  RNBubbleChartComponentView.mm
//  Fabric ComponentView wrapper for `RNBubbleChartView` (Swift).
//
...
@interface RNBubbleChartComponentView : RCTViewComponentView
@end

@implementation RNBubbleChartComponentView {
  RNBubbleChartView *_swiftView;
}

+ (ComponentDescriptorProvider)componentDescriptorProvider
{
  return concreteComponentDescriptorProvider<RNBubbleChartComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const RNBubbleChartProps>();
    _props = defaultProps;

    _swiftView = [[RNBubbleChartView alloc] initWithFrame:self.bounds];
...
{
  const auto &newProps = *std::static_pointer_cast<RNBubbleChartProps const>(props);
  const auto &oldP = oldProps
                         ? *std::static_pointer_cast<RNBubbleChartProps const>(oldProps)
                         : RNBubbleChartProps{};
...
Class<RCTComponentViewProtocol> RNBubbleChartComponentViewCls(void)
{
  return RNBubbleChartComponentView.class;
}
